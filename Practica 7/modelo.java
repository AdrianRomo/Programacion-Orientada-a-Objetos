...
class modelo extends AbstractTableModel {
    ...
    private Connection con;             // Conexion con base de datos
    ...
    **********************************************************************/
    public boolean conexion(  String driver, String host_basedatos, String tabla, String login, String pwd ) {
...
//    	    Class.forName("org.gjt.mm.mysql.Driver");  // Driver de mySQL
    ...
    /**************************************************************
     Conecta a la base de datos senalada en el parametro.
     Devuelve:
       - true: todo ha ido bien
       - false: hay un error. El mensaje de error se registra en atributo mensaje_error
     Para Access en local:
       1-Crear vista.policy en el directorio donde esta la pagina html con:
          grant {
     ...
     Para un applet local y MySQL en remoto:
       ...
       2-Tambion tiene que haber referencia a vista.policy en jre/lib/security/java.security
	  policy.url.n=file:c:/doc/java/jdbc02/class/vista.policy
     ****************************************************************/
...
//	    con = DriverManager.getConnection( "jdbc:mysql://proactiva-calidad.com:3306/" + base, "proactiv_alumnos", password);
	    ...
	    mensaje_error = new String( "******Error de conexion. " + e.getMessage() );
	    return false;
	}
    }
    ...
    ************************************************/
	...
	try {
	    ...
	    mensaje_error = new String( "No se pueden cargar los datos. Probablemente no hay conexion" );
	    return false;
	}
	catch (SQLException e) {
 ...
 void listarTablas(JComboBox cbbTablas) {
 try{
	...
	DatabaseMetaData meta = con.getMetaData();
      ...
      while (res.next()) {
         ...
         cbbTablas.addItem(res.getString("TABLE_NAME"));
      }
      res.close();
   } catch (SQLException e) {
	...
	try {
	    if ( rs != null ) {
		rs.last(); // Nos situamos en la ultima fila
		return rs.getRow(); // Devolvemos el numero de la fila
	    }
	    return 0;
	}
	catch(SQLException e) {  e.printStackTrace();  return 0;  }
